    ...
    async def push_leads(
        ...
        for lead in leads:
            ...
            if provider == "hubspot":
                ...
                if not api_key:
                    logger.info("HUBSPOT_API_KEY not configured, skipping push")
                logger.info("HubSpot payload: %s", payload)

            elif provider == "gohighlevel":
                api_key = self._env.get("GOHIGHLEVEL_API_KEY")
                if not api_key:
                    logger.info("GOHIGHLEVEL_API_KEY not configured, skipping push")
                logger.info("GoHighLevel payload: %s", payload)

            elif provider == "pipedrive":
                api_key = self._env.get("PIPEDRIVE_API_KEY")
                if not api_key:
                    logger.info("PIPEDRIVE_API_KEY not configured, skipping push")
                logger.info("Pipedrive payload: %s", payload)
